...
class FaqThemesTable extends Component implements HasForms, HasTable
{
    ...
    {
        $newModel = new FaqTheme();
        return $table
            ->query(FaqTheme::query())
            ->columns([
            ...
            ->defaultSort('id', 'desc')
            ->recordUrl(
                fn (Model $record): string => route('faq-themes.show', ['faq_theme' => $record]),
            )
            //->striped()
