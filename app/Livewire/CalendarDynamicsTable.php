...
class CalendarDynamicsTable extends Component implements HasForms, HasTable
{
    ...
    {
        $newModel = new CalendarDynamic();
        return $table
            ->query(CalendarDynamic::query())
            ->columns([
            ...
            TextColumn::make("text")
                ->label($newModel->getAttributeLabel("text"))
                ->sortable()
                ->toggleable()
                ->searchable(),
            ...
            ->actions([
                Action::make('edit')
                ->label(__('Update'))
                ->url(fn (CalendarDynamic $record): string => route('calendar-dynamics.edit', ['calendar_dynamic' => $record]))
            ...
            ->defaultSort('id', 'desc')
            ->recordUrl(
                fn (Model $record): string => route('calendar-dynamics.show', ['calendar_dynamic' => $record]),
            )
            //->striped()
    ...
    {
        return view('calendar_dynamics.table');
    }
}
