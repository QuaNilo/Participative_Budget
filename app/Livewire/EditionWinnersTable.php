...
class EditionWinnersTable extends Component implements HasForms, HasTable
{
    ...
    {
        $newModel = new EditionWinner();
        return $table
            ->query(EditionWinner::query())
            ->columns([
            ...
            ->actions([
                Action::make('edit')
                ->label(__('Update'))
                ->url(fn (EditionWinner $record): string => route('edition-winners.edit', ['edition_winner' => $record]))
            ...
            ->defaultSort('id', 'desc')
            ->recordUrl(
                fn (Model $record): string => route('edition-winners.show', ['edition_winner' => $record]),
            )
            //->striped()
    ...
    {
        return view('edition_winners.table');
    }
}
