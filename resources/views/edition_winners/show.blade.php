<x-app-layout>
    @section('breadcrumbs')
        {{ Breadcrumbs::render('edition-winners.show', $editionWinner) }}
    @endsection
    <div class="intro-y mt-8 flex flex-col items-center sm:flex-row">
        <h2 class="mr-auto text-lg font-medium">{{ __('Edition Winner Details') }}</h2>
        <div class="mt-4 flex w-full sm:mt-0 sm:w-auto">
            <x-base.button
                class="mr-2 shadow-md ml-auto sm:ml-0"
                variant="primary"
                href="{{ route('edition-winners.edit', $editionWinner) }}"
                as="a"
                >
                <x-base.lucide
                        class="mr-2 h-4 w-4"
            ...
            <form method="POST" action="{{ route('edition-winners.destroy', $editionWinner) }}" id="delete-record-form">
                @csrf
                @method('DELETE')
            </form>
        </div>
    ...
    <div class="intro-y box mt-3 p-5">
        <dl class="space-y-4">
            @include('edition_winners.show_fields')
        </dl>
    </div>
</x-app-layout>
