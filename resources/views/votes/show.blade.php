<x-app-layout>
    @section('breadcrumbs')
        {{ Breadcrumbs::render('votes.show', $vote) }}
    @endsection
    <div class="intro-y mt-8 flex flex-col items-center sm:flex-row">
        <h2 class="mr-auto text-lg font-medium">{{ __('Vote Details') }}</h2>
        <div class="mt-4 flex w-full sm:mt-0 sm:w-auto">
            ...
            <form method="POST" action="{{ route('votes.destroy', $vote) }}" id="delete-record-form">
                @csrf
                @method('DELETE')
            </form>
        </div>
